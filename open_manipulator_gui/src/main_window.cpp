...
#include <QMessageBox>
#include <iostream>
#include "open_manipulator_gui/main_window.hpp"
  ...
  , qnode(argc,argv)
{
  ...
  joint_name.push_back("grip_joint");
...
void MainWindow::on_des_joint_button_clicked( bool check )
{
  ...
  for ( int _id = 0; _id < joint_spinbox.size(); _id++ )
  {
    ...
    msg.position.push_back( ((QDoubleSpinBox *) joint_spinbox[ _id ])->value() * M_PI / 180.0 );
  }
...
void MainWindow::on_des_pos_button_clicked( bool check )
{
  ...
  msg.position.x = ui.pos_x_spinbox->value();
  msg.position.y = ui.pos_y_spinbox->value();
  msg.position.z = ui.pos_z_spinbox->value();
  ...
  msg.orientation.x = QR.x();
  msg.orientation.y = QR.y();
  msg.orientation.z = QR.z();
  msg.orientation.w = QR.w();
...
void MainWindow::on_write_button_clicked( bool check )
{
 // ROS_INFO("1");
  std_msgs::String msg;
  msg.data = "write";

  qnode.sendWriteMsg( msg );
}
