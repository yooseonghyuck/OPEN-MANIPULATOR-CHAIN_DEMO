...
void presentJointStatesCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
  ...
  for (int index = 0; index < msg->name.size(); index++)
  {
    ...
    if (present_msg.name[index] == "grip_joint")
    {
      present_msg.name.push_back("grip_joint_sub");
...
void goalJointStatesCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
  ...
  for (int index = 0; index < msg->name.size(); index++)
  {
    ...
    if (goal_msg.name[index] == "grip_joint")
    {
      goal_msg.name.push_back("grip_joint_sub");
...
{
  ros::init(argc, argv, "open_manipulator_chain_rviz_publisher");
  ros::NodeHandle nh("~");

  g_present_joint_states_pub = nh.advertise<sensor_msgs::JointState>("/robotis/open_manipulator_chain/present_joint_states", 0);
  g_goal_joint_states_pub = nh.advertise<sensor_msgs::JointState>("/robotis/open_manipulator_chain/goal_joint_states", 0);
